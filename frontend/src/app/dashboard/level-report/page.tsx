...
import { Users, TrendingUp, CheckCircle2, AlertTriangle, Loader2, BarChart3 } from 'lucide-react'
...
export default function LevelReportPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await http.get<LevelStats>('/dashboard/level-report', { schoolId })
      setStats(data)
    } catch (err) {
      console.error('Failed to load level report:', err)
      setError(true)
    } finally {
      setLoading(false)
    }
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-gray-900">
          {lang === 'ar' ? stats.levelNameAr : stats.levelName} - {t('Report', 'التقرير')}
        </h1>
        <p className="mt-2 text-sm text-gray-600">
          {t('Comprehensive level-wide performance overview', 'نظرة عامة شاملة على أداء المستوى')}
        </p>
      </div>
      ...
      <div className="grid gap-4 mb-8 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-gray-600 uppercase">{t('Groups', 'المجموعات')}</p>
              <p className="mt-2 text-2xl font-bold text-gray-900">{stats.totalGroups}</p>
            </div>
