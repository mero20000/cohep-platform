...
<title>{lang === 'ar' ? 'خدمتي' : 'My Ministry'} — Coptic Orthodox Hymn Education Platform (COHEP)</title>
   ...
   <motion.div className="space-y-6" initial="initial" animate="animate" variants={stagger}>
     ...
     >
      ...
      {/* My Group · Servants */}
      {groupMates !== null && (
        <motion.div variants={fadeUp} className="rounded-2xl border border-gray-200/60 bg-white p-5">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="font-semibold text-gray-900">{lang === 'ar' ? 'خدام مجموعتي' : 'My Group · Servants'}</h2>
            <span className="text-xs text-gray-500">{groupMates.length}</span>
          </div>
          {groupMates.length === 0 ? (
            <p className="text-sm text-gray-500">{lang === 'ar' ? 'لا يوجد خدام آخرون في مجموعتك' : 'No other servants in your group'}</p>
          ) : (
